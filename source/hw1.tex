...
\end{equation}
(5 points)
\item Let $\rho_1$ be a bimodal Gaussian distribution, $\rho_1(x) = w_1 \: (2\pi)\: |\Sigma_1|^{-1/2}\: e^{-(x-\mu_1)^\top \Sigma_1^{-1} (x-\mu_1)} + 
w_2 \: (2\pi)\: |\Sigma_2|^{-1/2}\: e^{-(x-\mu_2)^\top \Sigma_1^{-1} (x-\mu_2)},$ with the following values:
\begin{equation}
...
\begin{itemize}
    ...
    \item Set $\log \rho_t(x_1) = 0$ and solve the log probability equation \eqref{eq:logprobability} backward in time. The solution at time $t= 0$ is given by $\log\rho_0(x_0),$ where $\varphi^1(x_0) = x_1.$ You can evaluate $\log\rho_0$ anywhere since $\rho_0$ is a Gaussian distribution. 
    \item Then, recognize from \eqref{eq:}
\end{itemize}
\end{enumerate}
